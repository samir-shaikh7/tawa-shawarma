    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          let finalAddress = `Google Maps Pin: \n${mapsLink}`;
          ...
          try {
            ...
            console.warn("Could not reverse geocode:", geocodeErr);
            // We still have the coordinates, so we don't alert failure here
          }
          
          setFormData(prev => ({ ...prev, address: finalAddress }));
        } catch (err) {
          alert("Error processing location. Please enter manually.");
        } finally {
          setIsLocating(false);
        }
      },
      (error) => {
        ...
        if (error.code === error.TIMEOUT) {
          alert("Location request timed out. Please try outside or enter manually.");
        } else {
          alert("Location access denied or unavailable. Please check your device settings.");
        }
        setIsLocating(false);
      },
